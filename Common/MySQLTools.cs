...
namespace WeakCurrent1.Common
{
    ...
    public static class MySQLTools
    {
        ...
        public static void FasSQLInsertRow(MySqlConnection conn, string nametable, FASJiSuanshuExcel fasinfo)
        {
        ...
        /// <returns></returns>
        public static bool FasSQLInsert(MySqlConnection conn,string nametable,List<FASJiSuanshuExcel> listInsert)
        {
            ...
            if (listInsert.Count > 0 )
            {
                ...
                for (int i = 0; i < list4.Count; i++)
                {
                    ...
                    {
                        //插入第i行
                        FasSQLInsertRow(conn, nametable, list4[i]);
        ...
        public static bool FasUpdate(MySqlConnection conn, string tablename ,List<FASJiSuanshuExcel> list3)
        {
            if (list3.Count > 0)
            {
                //对list3中的每行进行处理
        ...
        /// <param name="conn"></param>
        /// <param name="fasinfo"></param>
        public static void FasUpdateSQLRow(MySqlConnection conn, string nametable, FASJiSuanshuExcel fasinfo)
        {
            //更新行对应的MySQL命令
            string sql_Update =
                "UPDATE " + nametable +
        ...
        public static void FasUpdateSQLByKey(MySqlConnection conn, string nametable, List<FASJiSuanshuExcel> listUpdateToSQL)
        {
            try
            {
                //先判断输入的列表是否有效
                if (listUpdateToSQL.Count > 0)
                {
                    ...
                    for (int i = 0; i < listUpdateToSQL.Count; i++)
                    {
                        ...
                        //从SQL中筛选,当前主键是否存在
                        string sql1 = "SELECT COUNT(*)  FROM " + nametable + " WHERE IdKey =@IdKey ";
                        MySqlCommand cmd = new MySqlCommand(sql1, conn);
                        cmd.Parameters.AddWithValue("@IdKey", IdKey);
                        object result = cmd.ExecuteScalar();
                        ...
                        {
                            //第i行的修改
                            FasUpdateSQLRow(conn, nametable, listUpdateToSQL[i]);
                        }
                    } //end of for
                    //此批次数据处理全部完成后关闭数据库连接
                    conn.Close();
        ...
        /// <returns></returns>
        public static int FasSQLGetKey(MySqlConnection conn, string Id_Dianjing, string name_table)
        {
            int IdKey=0;
            //连接MySQL
        ...
        /// <returns></returns>
        public static List<int> FasSQLGetIdKeys(MySqlConnection conn, string tablename)
        {
            //创建返回值列表
            ...
            {
                //SQL查询指令
                string sql1 = "SELECT IdKey FROM " + tablename;
                //查询当前防火分区对应的主键
                MySqlCommand cmd = new MySqlCommand(sql1, conn);
